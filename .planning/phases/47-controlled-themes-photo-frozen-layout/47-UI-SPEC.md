...
主题 key 为渲染器内部稳定值；YAML 可继续接受当前中文主题值并在解析时确定映射。每个主题只有上述一个照片位置，禁止用户指定坐标、色值、字体或照片变体。
...
| Item gap | 同一 `list-entry` 内 2 mm；相邻条目 3 mm；模块间 5 mm。空字段不占间距 |
...
声明值均为 4 的倍数；A4 内部实际使用以毫米几何为准，下面 token 用于 Typst 宏、模块间距和照片装饰的等价实现。
