...
**Goal:** As a teaching-materials author, I want a fixed structured data contract and reviewable Markdown intermediate for the end-of-term submission package, so semester data can be checked before any Typst/PDF rendering happens.
**Mode:** mvp

**Depends on:** Phase 4

**Requirements:** ETM-01, ETM-02, ETM-03, ETM-04

**Success Criteria:**
1. `skills/end-of-term-teaching-materials/SKILL.md` defines when to use the skill, the expected source data, safety boundaries, verification path, and adapter notes for all required runtimes.
2. The skill includes a structured data contract for course, class, teacher, semester, teaching-hour, assessment, and submission metadata.
3. The skill includes a reusable Markdown intermediate template for the end-of-term package, with separate reviewable sections for documents and tables.
4. Missing, ambiguous, or lossy data is surfaced in the Markdown intermediate through explicit review markers instead of being silently filled.

### Phase 12: Fixed-Template Rendering and Verification

**Goal:** As a teaching-materials author, I want the end-of-term Markdown package rendered into fixed-template Typst/PDF and deterministic table artifacts, so the generated submission materials can be verified before delivery.
**Mode:** mvp

**Depends on:** Phase 11

**Requirements:** ETM-05, ETM-06, ETM-07, ETM-08

**Success Criteria:**
1. The skill includes fixed package templates and keeps template-specific resources under `skills/end-of-term-teaching-materials/`.
2. `skills/end-of-term-teaching-materials/scripts/` includes a script that converts structured data to Markdown, renders Markdown to Typst, and compiles Typst to PDF.
3. The renderer emits deterministic table artifacts alongside the PDF package in a reviewable format.
4. The provided fixture verifies data validation, Markdown generation, Typst generation, PDF compilation, table output, and required runtime adapter notes.
