...
5. **先稳定契约，后扩展自动化**：v1 先稳定目录、模板、示例和兼容性说明；脚手架、校验器、runtime harness、generated wrappers 和发布流程后续再加。
...
4. 只有当某个 runtime 的说明太长时，才用 `templates/adapter/runtime-adapter.md` 新增独立 adapter notes。
5. 如果技能已经能代表最佳实践，把一个精简版放进 `examples/`。
6. 更新 `docs/compatibility-matrix.md` 中相关 runtime 的支持状态；不要在未验证时声称完整 runtime 支持。
