...
- `teaching-design-package`：用于整合授课计划和实操教案，先输出 `teaching-design-package-full.md` 结构的整包 Markdown intermediate，再把模块交给 `jiaoan-jihua` 与 `jiaoan-shicao` 并记录 split Typst/PDF status。Artifact contract 见 `skills/teaching-design-package/references/format-and-orchestration.md`。
