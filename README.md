...
- `teaching-design-package`：用于整合授课计划、实操教案和可选期末材料，先输出 `teaching-design-package-full.md` 结构的整包 Markdown intermediate，再把模块交给 `jiaoan-jihua`、`jiaoan-shicao` 与可选 `end-of-term-teaching-materials`，不替换原技能。入口见 `skills/teaching-design-package/SKILL.md`，Artifact contract 见 `skills/teaching-design-package/references/format-and-orchestration.md`，模板见 `skills/teaching-design-package/templates/teaching-design-package-full.md`，公开 helper 见 `skills/teaching-design-package/scripts/teaching-design-package.sh`。
