...
Presto Agent Skills 是一个面向多类 AI agent 的技能仓库框架。它让团队用统一的 canonical `SKILL.md` 编写技能，再通过 runtime adapter 说明不同 agent 的适配方式。
...
5. **先稳定契约，后扩展自动化**：v1 先稳定目录、模板、示例和兼容性说明，脚手架与校验器后续再加。
