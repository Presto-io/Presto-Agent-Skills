---
name: "jiaoan-jihua"
description: "Use when creating Chinese teaching-schedule or lesson-plan progress tables that must render through the Presto jiaoan-jihua Typst template."
metadata:
  short-description: "教案/授课进度计划 Markdown 到 Typst/PDF 工作流"
  version: "0.1.0"
...
把需要呈现为授课进度计划表的内容归一化为稳定的 `jiaoan-jihua-full.md` 结构，再通过 Presto `jiaoan-jihua` 模板生成可编译的 Typst 文件和 PDF 验证产物。
...
- `template_binary`: Presto jiaoan-jihua 模板可执行文件，默认路径为 `/Users/mrered/Library/Application Support/com.mrered.presto/templates/jiaoan-jihua/presto-template-jiaoan-jihua`；可用 `JIAOAN_JIHUA_TEMPLATE_BINARY` 或脚本参数覆盖。
...
5. 生成 Markdown 后，运行 `skills/jiaoan-jihua/scripts/jiaoan-jihua.sh render` 生成 `.typ`，必要时同时生成 `.pdf`。
...
skills/jiaoan-jihua/scripts/jiaoan-jihua.sh render \
  ...
  --pdf /path/to/jiaoan-jihua-full.pdf \
...
| Runtime | Notes |
|---------|-------|
| Codex | 读取本 `SKILL.md` 后执行流程；用 shell 调用 `scripts/jiaoan-jihua.sh`；写文件前确认目标路径，验证时运行 `typst compile`。 |
...
- 由 Presto `jiaoan-jihua` 模板生成的 `.typ` 文件。
- 由 Typst 编译生成的 `.pdf` 验证产物。

## Verification

- [ ] `skills/jiaoan-jihua/scripts/jiaoan-jihua.sh example --output <file>` 能输出可审阅的 `jiaoan-jihua-full.md` 结构。
- [ ] `skills/jiaoan-jihua/scripts/jiaoan-jihua.sh render --input <md> --typ <typ>` 能通过 Presto jiaoan-jihua 模板生成 Typst。
...
- 脚本能从 `jiaoan-jihua-full.md` 生成与 Presto 模板一致的 Typst，并编译出 PDF。
