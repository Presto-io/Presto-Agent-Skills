...
把需要呈现为授课进度计划表的内容归一化为稳定的 `jiaoan-jihua-full.md` Markdown intermediate，再通过技能自带 shell 脚本生成可编译 Typst。
...
- `references/calendar.json`: 教学日历/校历支持资源；当源材料依赖日期、周次或节假日排布时，先用它对齐教学进度。
- `output_markdown`: 持久化的 `jiaoan-jihua-full.md` Markdown intermediate，默认结构见 `templates/jiaoan-jihua-full.md`。
- `references/format-and-rendering.md`: frontmatter 字段、正文结构、校历规则、渲染约束和详细验证规则。
...
| Runtime | Notes |
|---------|-------|
| Codex | 读取本 `SKILL.md`、`references/format-and-rendering.md` 和必要的 `references/calendar.json` 后执行流程；用 shell 调用 `scripts/jiaoan-jihua.sh`；写文件前确认目标路径；PDF 编译若需要，必须在脚本外单独执行。 |
