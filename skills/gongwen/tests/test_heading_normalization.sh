...
"$RENDERER" render \
  ...
  --typ "$TMP_DIR/clean/heading.typ" >/dev/null
"$RENDERER" render \
  --input "$FIXTURE_DIR/heading-normalization-numbered.md" \
  --typ "$TMP_DIR/numbered/heading.typ" >/dev/null

cmp -s "$TMP_DIR/clean/heading.typ" "$TMP_DIR/numbered/heading.typ" || {
  printf '带手写序号和不带序号的标题渲染结果不一致。\n' >&2
  diff -u "$TMP_DIR/clean/heading.typ" "$TMP_DIR/numbered/heading.typ" >&2 || true
  exit 1
}
...
if grep '^#let FONT_SONG' "$TMP_DIR/clean/heading.typ" | grep -Eq 'SimHei|STHeiti|Sans|黑体'; then
  printf '宋体 fallback 中出现了黑体类字体。\n' >&2
  exit 1
fi

if grep '^#let FONT_XBS' "$TMP_DIR/clean/heading.typ" | grep -Eq 'SimSun|STSong|Songti|Serif|宋体'; then
