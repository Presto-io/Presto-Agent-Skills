...
以上十个 YAML formatter 值全部必须是字符串标量。日期样式、布尔值、数字、序列和映射不得以 YAML 隐式类型进入逻辑模型；有这类字面值时必须加引号或改写为字符串。例如必须写 `date: "2026-07-13"`，不能写 `date: 2026-07-13`。

D-02 锁定以上白名单。D-03 规定：YAML 未提供 `title` 时，正文中第一个且唯一的文档级 `#` 作为标题回退；YAML 已有 `title` 时它不覆盖 YAML。文档级 `#` 最多一个，且永不进入目录。目录只收集 `layout="section"` 幻灯片内的 `##`，不收集三级内容页标题，不添加项目编号，并严格保持源码顺序。
...
D-05 定义完整段落、完整列表、单张图片、完整表格和 fenced code 为原子顶层块。空行结束段落；连续列表项组成一个块；只有“表头 + 合法 Markdown 分隔行 + 全部数据行”才能组成一个完整表格块。分隔行和每个数据行必须与表头列数相同；`table` 与 `timeline` 都遵守此结构约束。
...
| `contents` | 显式空容器；条目仅由 `section` 的 `##` 自动生成且不编号 |
| `section` | 一个 `##`，不得有正文 |
...
D-15 采用允许列表：fenced code 外拒绝 raw HTML、HTML 注释、通用 attributes、`id`、coordinates、`x/y`、width/height/dimensions、crop、footer、font/font-size、color/background 和 style 控制。任何未知 `:::` directive 也属于错误。fenced code 内容完全不透明，其中独立的三冒号行、`::: notes`、`::: slide {layout="..."}`、HTML、CSS、标题、坐标、字体或颜色字样都只作为逐行代码文本保留；只有同字符且长度不少于 opener 的 fence 才能闭合代码块。
...
普通 Markdown 中的 `{{待补充: ...}}` 与 `{{AI草稿: ...}}` 是未解决审阅标记，`validate` 与 `render` 均以 `REVIEW_MARKER_UNRESOLVED` 非零退出；fenced code 中的相同字面量保持不透明，不触发该诊断。
