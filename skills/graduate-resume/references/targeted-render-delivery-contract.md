...
- **D-06 四态：** 每个 condition 固定为 `meets`、`gap`、`unknown`、`not-applicable` 之一。公开预检显示 condition ID、受控谓词、状态、计数和 digest；完整 reason 与 evidence fact IDs 只写 owned hidden evidence。
...
- **D-16 确认前预检：** 公开输出显示权威版本矩阵、完整 safe stems、page/photo/source/as_of、逐条件四态、gap allow，以及 `added/updated/unchanged/removed`。没有 `--confirm` 时不改变 current/history。
